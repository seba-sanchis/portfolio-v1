  ...
  const handleClick = (e, i) => {
    ...
    if (carouselRef.current) {
      const scrollLeft = Math.floor(
        carouselRef.current.scrollWidth * 0.7 * (i / education.length)
      );
    ...
    if (carouselRef.current) {
      const index = Math.round(
        (carouselRef.current.scrollLeft /
          (carouselRef.current.scrollWidth * 0.9)) *
          education.length
      );
  ...
  return (
    <Section id="skills">
      ...
      <List>
        {/* First List */}
        <ListItems>
          <ListContainer role={"button"} tabIndex={0} onClick={() => setToggleList1((state) => !state)}>
        ...
        {/* Second List */}
        <ListItems>
          <ListContainer role={"button"} tabIndex={0} onClick={() => setToggleList2((state) => !state)}>
        ...
        {/* Third list */}
        <ListItems>
          <ListContainer role={"button"} tabIndex={0} onClick={() => setToggleList3((state) => !state)}>
